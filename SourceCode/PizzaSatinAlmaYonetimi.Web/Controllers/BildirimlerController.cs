...
[Authorize]public sealed class BildirimlerController(IBildirimService service,ILogger<BildirimlerController> logger):Controller
{
 ...
 [HttpPost][ValidateAntiForgeryToken]public async Task<IActionResult>OkunduYap(int bildirimId,CancellationToken ct){try{await service.OkunduYapAsync(bildirimId,Id(),ct);TempData["Basari"]="Bildirim okundu olarak işaretlendi.";}catch(Exception ex){logger.LogError(ex,"Bildirim okundu yapılamadı.");TempData["Hata"]="Bildirim güncellenemedi.";}return RedirectToAction(nameof(Index));}
